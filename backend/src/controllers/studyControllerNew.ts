...
import { RequestWithPrisma, CreateStudyDto, UpdateStudyDto, ApiResponse } from '../types';
...
export const getStudies = async (
  ...
  try {
    const studies = await (req.prisma as any).study.findMany({
      include: {
        creator: {
...
export const getStudyById = async (
  ...
  try {
    ...
    const study = await (req.prisma as any).study.findUnique({
      where: { id },
      include: {
        creator: {
          select: { id: true, firstName: true, lastName: true, email: true }
...
export const createStudy = async (
  ...
  try {
    ...
    const study = await (req.prisma as any).$transaction(async (prisma: any) => {
      ...
      if (protocol.steps && protocol.steps.length > 0) {
        await Promise.all(protocol.steps.map((step: any, index: number) => 
          prisma.studyDataCollectionStep.create({
            data: {
              studyId: newStudy.id,
              stepNumber: index + 1,
...
export const updateStudy = async (
  ...
  try {
    ...
    const existingStudy = await (req.prisma as any).study.findUnique({
      where: { id },
    ...
    // Aktualizuj badanie
    const updatedStudy = await (req.prisma as any).study.update({
      where: { id },
      data: {
        name: data.name,
        description: data.description,
...
export const deleteStudy = async (
  ...
  try {
    ...
    await (req.prisma as any).study.delete({
      where: { id }
    });
